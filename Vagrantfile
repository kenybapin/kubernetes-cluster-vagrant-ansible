...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "k8snode" do |k8snode|
    k8snode.vm.box = "bento/ubuntu-18.04"
    k8snode.vm.hostname = 'k8snode'
    k8snode.vm.provision "docker"
    k8snode.vm.box_url = "bento/ubuntu-18.04"

    k8snode.vm.network :private_network, ip: "192.168.56.201"

    k8snode.vm.provider :virtualbox do |v|
      v.customize ["modifyvm", :id, "--natdnshostresolver1", "on"]
      v.customize ["modifyvm", :id, "--memory", 2048]
      v.customize ["modifyvm", :id, "--name", "k8snode"]
      v.customize ["modifyvm", :id, "--cpus", "2"]
    end
